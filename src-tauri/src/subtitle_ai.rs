...
mod tests {
    ...
    #[test]
    fn parses_json_object_from_fenced_response() {
        let parsed = parse_json_text_map("```json\n{\"1\":\"你好\",\"2\":\"世界\"}\n```").unwrap();
        assert_eq!(parsed.get("1"), Some(&"你好".to_string()));
        assert_eq!(parsed.get("2"), Some(&"世界".to_string()));
    }

    #[test]
    fn parses_last_json_object_from_reasoning_response() {
        let response = r#"分析过程里可能会提到 {"1":"原文"}。
最终输出:
{"1":"校正后","2":"字幕 {内容}"}"#;

        let parsed = parse_json_text_map(response).unwrap();

        assert_eq!(parsed.get("1"), Some(&"校正后".to_string()));
        assert_eq!(parsed.get("2"), Some(&"字幕 {内容}".to_string()));
    }

    #[test]
    fn validates_split_without_text_changes() {
        let source = "大家好今天我们来学习机器学习";
        let sentences = vec!["大家好".to_string(), "今天我们来学习机器学习".to_string()];

        assert!(validate_split_result(source, &sentences).is_ok());
    }

    #[test]
    fn rejects_split_text_changes() {
        let source = "hello world";
        let sentences = vec!["hello brave world".to_string()];

        assert!(validate_split_result(source, &sentences).is_err());
    }
    ...
    fn merges_word_units_by_sentence_lengths() {
        ...
        let sentences = vec!["大家好".to_string(), "今天".to_string()];
        let segments = merge_word_units_by_sentences(&words, &sentences);

        assert_eq!(segments.len(), 2);
        assert_eq!(segments[0].start_time, 0);
        assert_eq!(segments[0].end_time, 700);
        assert_eq!(segments[1].start_time, 700);
        assert_eq!(segments[1].end_time, 1200);
    }

    #[test]
    fn skips_single_short_segment_for_split_ai() {
        let segment = TranscriptionSegment {
            text: "The final short subtitle".to_string(),
            start_time: 0,
            end_time: 1200,
            uid: "raw-0".to_string(),
        ...
        let mut blocks = build_segmentation_blocks(&[segment]);

        let skipped = mark_blocks_without_split_needed(&mut blocks);
