...
pub fn set_download_progress_callback(cb: impl Fn(u64, u64) + Send + 'static) {
    let callbacks = DOWNLOAD_PROGRESS_CB.get_or_init(|| Mutex::new(None));
    if let Ok(mut callback) = callbacks.lock() {
...
pub fn set_split_progress_callback(cb: impl Fn(SplitProgress) + Send + 'static) {
    let callbacks = SPLIT_PROGRESS_CB.get_or_init(|| Mutex::new(None));
    if let Ok(mut callback) = callbacks.lock() {
        *callback = Some(Box::new(cb));
    }
}
